...
export default function StyledCarousel({ slideList, unoptimized }) {
  ...
  const plusSlide = () => {
    const newSlide =
      currentSlide == slideList.length - 1 ? 0 : currentSlide + 1;
    setCurrentSlide(newSlide);
  };
  const lessSlide = () => {
    const newSlide =
      currentSlide == 0 ? slideList.length - 1 : currentSlide - 1;
    setCurrentSlide(newSlide);
  };

  slideList.map((value, index) => {
    const source = value.includes("http")
      ? value
      : `${process.env.BACK_END}/image/${value}`;
    mySlideList.push({
      key: index,
      content: (
        <Image
  ...
  return (
    <CustomCarousel>
      ...
      <div className="Dots">{myDotList}</div>
    </CustomCarousel>
  );
}
